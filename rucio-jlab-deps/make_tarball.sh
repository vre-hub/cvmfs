...
run_install () {
  ...
  pip install -r ../requirements-rucio-jlab.txt  # Install Rucio jlab dependencies - script run inside cd $RUCIO_EXT_VERSION
...
tar zcf ../rucio-jlab-deps-$RUCIO_EXT_VERSION.tar.gz *

echo "General: DONE!"

# Provide the command to upload the tarball to a remote server
# echo "scp rucio-jlabs-deps-$RUCIO_EXT_VERSION.tar.gz ${USER}@lxplus:~/public/rucio-clients/"
